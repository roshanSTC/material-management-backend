...
class SupplierInvoiceItemCreateSchema(Schema):
    class Meta:
        unknown = EXCLUDE
...
class SupplierInvoiceItemUpdateSchema(Schema):
    class Meta:
        unknown = EXCLUDE
...
class SupplierInvoiceItemResponseSchema(Schema):
    class Meta:
        unknown = EXCLUDE

    id = fields.Integer(required=True)
    supplier_invoice_id = fields.Integer(required=True)
...
class SupplierInvoiceCreateSchema(Schema):
    class Meta:
        unknown = EXCLUDE
    ...
    def normalize_invoice_data(self, data, **kwargs):
        ...
        if resolved_date is not None:
            idate = _parse_date(resolved_date)
            if idate:
                normalized["invoice_date"] = idate.isoformat()
...
class SupplierInvoiceUpdateSchema(Schema):
    class Meta:
        unknown = EXCLUDE
    ...
    def normalize_update_data(self, data, **kwargs):
        ...
        if "invoiceDate" in normalized and "invoice_date" not in normalized:
            normalized["invoice_date"] = normalized["invoiceDate"]
        ...
        if "payment_term" in normalized and "payment_terms" not in normalized:
...
class SupplierInvoiceQuerySchema(Schema):
    class Meta:
        unknown = EXCLUDE
...
class SupplierInvoiceResponseSchema(Schema):
    class Meta:
        unknown = EXCLUDE
    ...
    items = fields.List(
        fields.Nested(SupplierInvoiceItemResponseSchema),
        required=True,
    )
